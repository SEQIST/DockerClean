...
import { Department } from '../types/Department'; // Import der gemeinsamen Typen
...
const DepartmentImport: React.FC<DepartmentImportProps> = ({ importFile, setImportFile, setError, setDepartments, departments }) => {
    ...
    try {
      if (fileExtension === 'json') {
        ...
        reader.onload = async (e) => {
          try {
            departmentsData = JSON.parse(e.target?.result as string);
        ...
        reader.onload = async (e) => {
          try {
            ...
            departmentsData = lines.map(line => {
              const parts = line.split(',').map(part => part.trim());
              const dept: any = {};
              parts.forEach(part => {
                const [key, value] = part.split(':').map(item => item.trim());
                if (key && value) {
                  if (key.toLowerCase() === 'name') dept.name = value;
                  if (key.toLowerCase() === 'beschreibung') dept.description = value;
                }
              });
              return dept;
  ...
  const importDepartments = async (departmentsData: any[]) => {
    if (!Array.isArray(departmentsData)) {
      setError('Erwartet ein Array von Abteilungen');
      return;
    }

    try {
      const response = await fetch('http://localhost:5001/api/departments/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(departmentsData),
      });
      if (!response.ok) throw new Error('Fehler beim Importieren der Abteilungen');
      const importedDepartments = await response.json();
      setDepartments([...departments, ...importedDepartments]);
      setImportFile(null);
    } catch (error) {
      setError('Fehler beim Importieren: ' + (error as Error).message);
    }
